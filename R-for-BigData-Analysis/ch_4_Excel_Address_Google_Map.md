...
#### 2-2. `센터명`과 `주소` 열의 데이터 타입이 factor 형. 이를 문자형으로 변환한다. (`as.character()` 함수 사용)
...
#### 2-3. 구글 API 인증 및 주소록의 geocode 다운로드
...
#### 2-5. 센터명과 geocode로 구성된 데이터 프레임 변수 `df`를 생성한다.
...
#### 2-7. 구글 지도정보 확인 및 구글 지도에 센터  위치 표시하기
